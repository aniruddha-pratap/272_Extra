...
data<-read.csv('272_CO_DailyData.csv')
data_file<-read.csv('272_CO_DailyData.csv')
head(data)
head(data_file)
...
ggplot(data_file) + geom_point(aes(x = date, y = data_file$DAILY_AQI_VALUE, colour =  data_file$DAILY_AQI_VALUE<50)) + 
	scale_colour_manual(name= 'CO < 50', values = setNames(c('green','red'), c(T, F))) + xlab('Month') + ylab('CO')
